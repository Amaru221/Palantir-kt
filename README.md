...
* ⚙️ **On-Device Settings & Filtering:** Dedicated settings menu to customize your Gemini API Key, filter TTS voices by language (ES/US) and gender (Male/Female), and tweak speech rate and pitch.


## How It Works

1. **Wake Word Listening:** The smartwatch listens in standby mode for the phrase *"Oye Palantir"* (or *"Oye Palantir"* in US mode).
...
4. **Speech Synthesis:** Text-To-Speech (TTS) dictates the AI's response aloud using configured pitch, speed, and filtered Google voice settings.
...
* **Language Selector (ES / US):** Toggle speech synthesis and wake word listening language between Spanish (`es-ES`) and English (`en-US`).
* **Gender Selector (Hombre / Mujer):** Dynamically filter available Google TTS voices by Male or Female voice profiles.
* **Filtered Voice List:** Easily select from the voices available matching your chosen language and gender options.
* **TTS Speed & Pitch Controls:** Fine-tune voice speed (`speechRate`) and tone (`pitch`) using native Wear OS sliders.
* **Persistent Preferences:** All settings are saved locally in `SharedPreferences` and restored automatically across app restarts.